...
roll_max <- function( x, n, by.column=TRUE, weights=rep(1,n), normalize=FALSE ) {
	...
	if( is.matrix(x) ) {
		...
		return( .Call( "RcppRoll_roll_max_numeric_matrix", x, as.integer(n), as.logical(by.column), as.numeric(weights), PACKAGE="RcppRoll" ) )
	}
	...
	if( is.vector(x) ) {
		...
		return( as.numeric( .Call( "RcppRoll_roll_max_numeric_vector", x, as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) ) )
	}
